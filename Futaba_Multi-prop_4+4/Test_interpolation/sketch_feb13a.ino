...
volatile int multi_output_crt[8]; // last values
...
void setup()
{
  ...
  for (int outputnum = 0; outputnum < 6; outputnum++)
  {
    ...
    else
    {
      ...
      multi_output_crt[outputnum] = 1500;
 ...
 // if (output_ready == 1)
  ...
  if (output_ready == 1)
  {
    for (outputnum = 0; outputnum < 8; outputnum++)
    {
      ...
      if (outputnum == 0)
      {
        value = multi_output_crt[outputnum];
        increment = (multi_output[outputnum] - multi_output_crt[outputnum]) / 10;
        for(i = 0; i<10; i++)
        {
          value += increment;
          servos[outputnum].write(value);
          delay(5);
        }
        multi_output_crt[outputnum] = multi_output[outputnum];
...
void calcAux()
{
  ...
  { //Filter HIGH pulse If time between 700 and 2200 is a HIGH pulse
    ...
    else
    {
      ...
      {
        Serial.print("lost sync pulse, current_output=");
